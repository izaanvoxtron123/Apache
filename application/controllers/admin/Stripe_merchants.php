...
class Stripe_merchants extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('knowledge_base_model');
        $this->load->model('stripe_merchants_model');
    }

    /* List all knowledgebase articles */
    public function index()
    {
        if (!has_permission('stripe_merchants', '', 'view')) {
            access_denied('stripe_merchants');
        }
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('stripe_merchants');
        }
        $data['groups']    = $this->knowledge_base_model->get_kbg();
        $data['bodyclass'] = 'top-tabs kan-ban-body';
        $data['title']     = _l('kb_string');
        $this->load->view('admin/stripe_merchants/list', $data);
    }

    public function add($id = '')
    {
        if (!has_permission('stripe_merchants', '', 'add')) {
            access_denied('stripe_merchants');
        }
        ...
        if ($this->input->post()) {
            ...
            $response = $this->stripe_merchants_model->add($data);

            if ($response['status']) {
                $id = $response['id'];
                if ($id) {
                    set_alert('success', _l('added_successfully', 'Stripe Merchant'));
                    redirect(admin_url('stripe_merchants/edit/' . $id));
                }
            } else {
        ...
        if ($id == '') {
            $title = _l('add_new', 'stripe merchant');
        } else {
            $title = _l('edit', 'stripe merchant');
        }
        ...
        $this->load->view('admin/stripe_merchants/form', $data);
    }
    ...
    public function edit($id)
    {
        ...
        if (!has_permission('stripe_merchants', '', 'edit')) {
            access_denied('stripe_merchants');
        }

        if ($this->input->post()) {
            $data                = $this->input->post();
            $response = $this->stripe_merchants_model->edit($id, $data);

            if ($response['status']) {
                set_alert('success', _l('updated_successfully', 'Stripe Merchant'));
                redirect(admin_url('stripe_merchants/edit/' . $id));
            } else {
                redirect($_SERVER['HTTP_REFERER']);
            }
        ...
        $result = $this->stripe_merchants_model->get($id);
        $data['result'] = $result;
        $data['id'] = $id;
        $title = _l('edit', 'stripe merchant');
