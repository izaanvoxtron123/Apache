...
class Invoices extends REST_Controller
{
    ...
    public function data_post()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['allowed_payment_modes'] = [];

            if (count($payment_modes)) {
                foreach ($payment_modes as $payment_mode) {
                    array_push($data['allowed_payment_modes'], $payment_mode['id']);
                }
            }
            ...
            if ($id > 0 && !empty($id)) {
                // success
                $message = array('status' => TRUE, 'message' => 'Invoice Added Successfully', 'invoice' => $invoice);
